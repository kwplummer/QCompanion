 ...
 * \details Constructs the WaiterDialog, loads timers and starts updating timers.
...
{
    WaiterWidget *w = new WaiterWidget(this,ui->EntryDate->selectedDate(),ui->EntryTime->time(),ui->EventTitle->text());
    widgets.push_back(w);
...
/*!
 * \brief Updates all the Waiters with the current time, additionally sets the currentTimeLabel to the current time.
 */
...
/*!
 * \brief Sets the two fields used to set a timer's date and time to the current date and time.
 */
...
/*!
 * \brief Removes a waiter, and sets all the entry fields to what were used to create it.
 * \param[in] waiter What to remove
 * \param[in] name What the old waiter was called
 * \param[in] datetime When the waiter was waiting until
 */
void WaiterDialog::replaceWaiter(WaiterWidget *waiter, QString name, QDateTime datetime)
{
 ...
 * \details Saves all the waiters to disk. Called whenever a waiter is added or removed.
 * Additionally this updates the time the next speech should occur, and notifies the component.
 */
void WaiterDialog::commitChanges()
{
    if(!statePath.isNull())
    {
        std::sort(widgets.begin(),widgets.end(),[](const WaiterWidget *w1,const WaiterWidget *w2)
        {
 ...
 * \details Loads all the waiters from disk. Called on startup or when the save path changes.
 * Additionally this updates the time the next speech should occur, and notifies the component.
 */
    ...
    if(!statePath.isNull())
    {
        ...
        if(file.exists())
        {
            ...
            while(!in.atEnd())
            {
                ...
                if(!title.isEmpty())
                {
                    WaiterWidget *w = new WaiterWidget(this,time,title.replace("_"," ",Qt::CaseInsensitive));
                    widgets.push_back(w);
                    ui->TimerScrollerWidget->layout()->addWidget(w);
#if QT_VERSION < 0x050000
                    connect(w,SIGNAL(removeAt(WaiterWidget*)),this,SLOT(removeWaiter(WaiterWidget*)));
...
void WaiterDialog::updateNextTime()
{
    ...
    time_t earliest = 0;
    const std::vector<time_t> times{31536000000,2592000000,604800000,86400000,43200000,21600000,21600000,3600000,1800000,900000,600000,300000,60000,0};
