 ...
 */
WaiterWidget::WaiterWidget(QWidget *parent, QDate endDate, QTime endTime, QString title) :
    QWidget(parent),
    datetime(endDate,endTime),
    title(title.replace("_"," ")),
    initMs(QDateTime::currentDateTime().toMSecsSinceEpoch()),
    quit(false)
{
    init();
}
...
WaiterWidget::WaiterWidget(QWidget *parent, qint64 msec, QString title) :
    QWidget(parent),
    datetime(QDateTime::fromMSecsSinceEpoch(msec)),
    title(title),
    initMs(QDateTime::currentDateTime().toMSecsSinceEpoch()),
    quit(false)
{
 ...
 * \return A QString with the Waiter's title (or NULL) and the msecs since the UNIX epoch it will wait for.
 */
QString WaiterWidget::toLoggableString()
{
    const QString test = (title.isEmpty() ? "NULL" : title.replace(" ","_")) + ' ' + QString::number(datetime.toMSecsSinceEpoch());
    return((title.isEmpty() ? "NULL" : title.replace(" ","_")) + ' ' + QString::number(datetime.toMSecsSinceEpoch()));
}
 ...
 * Additionally if the time from t to the end of the timer is important, it notifies the dialog
 * (Important times are 1 year, 1 month, 1 week, 1 day, 12 hours, 6 hours, 1 hour, 30 minutes, 15 minutes, 10 minutes, 5 minutes, 1 minute, 0 seconds)
        ...
        switch(seconds)
        {
            case (31536000): //365 * seconds in a day = 1 year
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in one year";
            ...
            case(2592000):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in one month";
                emit speakThis(sayMe);
                break;
            }
            case(604800):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in one week";
                emit speakThis(sayMe);
                break;
            }
            case(86400):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in one day";
                emit speakThis(sayMe);
                break;
            }
            case(43200): //12 * seconds in an hour = 12 hours
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in 12 hours";
                emit speakThis(sayMe);
                break;
            }
            case(21600):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in 6 hours";
                emit speakThis(sayMe);
                break;
            }
            case(3600):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in an hour";
                emit speakThis(sayMe);
                break;
            }
            case(1800): //30 * seconds in one minute = 30 minutes
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in half an hour";
                emit speakThis(sayMe);
                break;
            }
            case(900):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in 15 minutes";
                emit speakThis(sayMe);
                break;
            }
            case(600):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in 10 minutes";
                emit speakThis(sayMe);
                break;
            }
            case(300):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in 5 minutes";
                emit speakThis(sayMe);
                break;
            }
            case(60):
            {
                ...
                if(title == "NULL" || title.isEmpty())
                    ...
                    sayMe = "The timer " + title.replace("_"," ") + " will expire in 1 minute";
                emit speakThis(sayMe);
                break;
            }
        }
...
/*!
 * \brief Tells the WaiterDialog to remove this Waiter. If shift is held down the dialog is instead told to replace the waiter.
 */
